...
(
    ...
    `type`    int not null  COMMENT '报考类型,0考，1保',
    `isDelete`    int           not null COMMENT '是否删除'
) COMMENT '招生计划';
...
(
    ...
    `admissionType` int  COMMENT '录取类型,0学，1专，2直',
    `volunteerNumber` int not null COMMENT '包含的志愿数目',
    `type`    int not null COMMENT '报考类型,0考，1保',
...
(
    ...
    `firstGrade`      double           not null COMMENT '初试得分',
    `secondGrade`      double           not null COMMENT '复试得分',
    `finalGrade`      double           not null COMMENT '最终得分',
